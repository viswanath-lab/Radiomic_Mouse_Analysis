function [featints,featnames,featstats,statnames] = extract2DFeatureInfo_haralick_moded(img,mask,class_options,ws_options)
    % Feature extraction for a series of 2D images
    %Haralick2d is being tested to reduce the number of bins to 
    %INPUTS:
    ...
    %stats_options=(OPTIONAL) which statistics will be calculated and
    %outputed.
%                 DEFAULT: ws_options = [3, 5, 7, 9, 11]
...
if ~all(cellfun('isempty',strfind(class_options,'haralick')))
    ...
    nbins = length(unique(img)); % a power of 2
...
if ~all(cellfun('isempty',strfind(class_options,'laws')))
    ...
    for ws = ws1:2:ws2
        if ws <= 5
            fprintf('\tUsing a window size of %i:\n',ws)
            [lf, lfn] = lawsfilter_c(img(:,:,i),ws,0);
